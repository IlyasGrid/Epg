<x-adminLayout>
    <header>
        <h2> <a href="/admin/diplomes/{{ $diplome->id }}/branche/"
                class="text-reset text-decoration-none text-capitalize">
                {{ $branche->Fullname }} <span class="text-muted text-capitalize"> programe </span>
            </a>
        </h2>
        <h4>
                    ...
                    <ul id="chapitres-list" class="list-group pl-5">
                        <li class="list-group-item mb-2 mt-1">
                            <input type="text" class="border border-gray-200 rounded p-2 w-full form-control"
                                name="Name_module[]" placeholder=" modules" value="{{ old('Name_module[]') }}" />
                        </li>
                    </ul>
                    <div class="d-flex justify-content-center">
                        <button class="add-chapitre-btn btn btn-outline-success"
                            onclick="addObjectif( '#chapitres-list', 'Name_module[]', ' modules');" type="button">
                            Add Module</button>
                    </div>
